...
public class FenUtil {
    ...
    public static String toFen(String[][] board) {
        ...
        // 1. 根据将帅宫格位置确定方向
        //    FEN标准: rank 10(第一行)=黑方阵地, rank 1(最后一行)=红方阵地
        //    红方在下: board[0]=黑方→FEN rank10(第一行), board[9]=红方→FEN rank1, 顺序0→9
        //    黑方在下: board[0]=红方→FEN rank1(最后一行), board[9]=黑方→FEN rank10, 顺序9→0
        ...
        for (int i = 0; i < 10; i++) {
            int row = redBottom ? i : (9 - i);
            StringBuilder rowBuilder = new StringBuilder();
            int emptyCount = 0;

            for (int j = 0; j < 9; j++) {
                String piece = board[row][j];
