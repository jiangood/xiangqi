package io.github.jiangood.xq.util;

import io.github.jiangood.xq.opencv.BoardUtils;
import java.util.Map;

/***
 * 进：棋子向对方方向（即棋盘上方）移动。
 * 退：棋子向己方方向（即棋盘下方）移动。
 * 平：棋子横向移动（左右移动，不改变纵线）。
 *
 * 用法示例：
 * convertToChineseNotation("e2e4")  → "炮五进四"
 * convertToChineseNotation("h2e2")  → "炮二平五"
 * convertToChineseNotation("h0g2")  → "马二进三"
 */
public class NotationConverter {


    // 中文棋谱从右到左
    private static final String COLUMN_LETTERS_CN = "一二三四五六七八九";
    private static final Map<Character, Character> PIECE_MAP = Map.of(
            'r', '车', 'n', '马', 'c', '炮',
            'b', '象', 'a', '士', 'k', '帅', 'p', '兵'
    );


    public static String convertToChineseNotation(String[][] board, String move) {
        if (move.length() != 4) {
            return "非法走法" + move;
        }

        // 解析起始和目标位置
        int x1 = move.charAt(0) - 'a';
        int y1 = move.charAt(1) - '0';

        // 根据棋盘方向确定引擎坐标到棋盘行的映射
        // 引擎坐标: y=0=红方底线, y=9=黑方底线(固定)
        // 棋盘数组: board[0]=图像顶部, board[9]=图像底部
        // 红方在下: 引擎y=0 → 图像底部 → board[9], 映射: 9 - y
        // 黑方在下(FEN已翻转): 引擎y=0 → 图像顶部 → board[0], 映射: y
        boolean redBottom = BoardUtils.isRedBottom(board);
        int boardRow = redBottom ? (9 - y1) : y1;

        // 获取棋子类型
        String piece = board[boardRow][x1];

        if (piece == null || piece.trim().isEmpty()) {
            return "起始位置无棋子";
        }

        char pieceType = piece.charAt(1); // 第二个字符是棋子类型
        String pieceName = String.valueOf(getChinesePieceName(pieceType));

        String suffix = convertToChineseNotation(move, redBottom);

        return pieceName + suffix;
    }

    /**
     * 参考图三
     * <p>
     * 坐标
     *
     * @param move
     * @return
     */
    public static String convertToChineseNotation(String move) {
        return convertToChineseNotation(move, true);
    }

    /**
     * 根据棋盘方向转换 UCI 走法为中文记谱
     * @param move      UCI 走法, 如 "e2e4"
     * @param redBottom true=红方在下方(标准), false=黑方在下方
     * @return 中文记谱, 如 "炮五进四"
     */
    private static String convertToChineseNotation(String move, boolean redBottom) {
        if (move.length() != 4) {
            return "非法走法" + move;
        }

        // 解析起始和目标位置, 坐标原点为左下
        int x1 = move.charAt(0) - 'a';
        int y1 = move.charAt(1) - '0';
        int x2 = move.charAt(2) - 'a';
        int y2 = move.charAt(3) - '0';

        // 中文（中文坐标从右到左，从1开始）
        char x1cn = COLUMN_LETTERS_CN.charAt(COLUMN_LETTERS_CN.length() - x1 - 1);
        char x2cn = COLUMN_LETTERS_CN.charAt(COLUMN_LETTERS_CN.length() - x2 - 1);

        // 平
        if (y1 == y2) { // 平，      H2-E2  炮二平五
            return x1cn + "平" + x2cn;
        }

        // 进退方向取决于哪方在下方:
        //   引擎坐标y=0=红方底线, y=9=黑方底线
        //   红方在下: y2>y1 → 向黑方(前进) → 进
        //   黑方在下: y2>y1 → 向黑方(后退) → 退
        boolean isForward = redBottom ? (y2 > y1) : (y2 < y1);
        String dir = isForward ? "进" : "退";
        int step = Math.abs(y2 - y1);
        char stepCn = COLUMN_LETTERS_CN.charAt(step - 1);

        // 进退， 判断字母相同
        if (x1 == x2) { //     map.put("E2-E6", "炮五进四");
            return x1cn + dir + stepCn;
        }

        // 如果是否斜着走，最后一个字为目标列号
        return x1cn + dir + x2cn;
    }

    public static char getChinesePieceName(char pieceType) {
        return PIECE_MAP.getOrDefault(pieceType, pieceType);
    }
}
