...
object AndroidImageUtils {
    ...
    fun renderBoardVisualization(
        ...
        if (bestMove != null && bestMove.length == 4) {
            ...
            // 红方在下: 引擎y=0 → 图像底部 → grid row 9, 映射: 9 - y
            // 黑方在下(FEN已翻转): 引擎y=0 → 图像顶部 → grid row 0, 映射: y
            val redBottom = BoardUtils.isRedBottom(board)
            val fromRow = if (redBottom) 9 - (bestMove[1] - '0') else (bestMove[1] - '0')
            val toRow = if (redBottom) 9 - (bestMove[3] - '0') else (bestMove[3] - '0')
